...
fun main(args: Array<String>) {
    calculateLiftTicks(weights, destinations, floors, maxPassengers, maxWeight, numberOfLifts, numberOfExpressLifts)
}
...
fun calculateLiftTicks(weights: Array<Int>, destinations: Array<Int>, floors: Int, maxPassengers: Int, maxWeight: Int,
                       numberOfLifts: Int, numberOfExpressLifts: Int) {
    ...
    // We need at least one of each lift type.
    if (numberOfLifts == 0 || numberOfExpressLifts == 0)
        throw IllegalArgumentException("must have at least one of each lift type")
    ...
    val liftStates = MutableList(numberOfLifts) { LiftState(lift) }
    liftStates.addAll(List(numberOfExpressLifts) { ExpressLiftState(lift) })
    ...
    val liftManager = LiftManager(liftStates, passengers)
